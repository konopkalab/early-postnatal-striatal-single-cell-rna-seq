...
In addition to main analysis for neuronal subset, code chunks for few additional analyses, plots is covered here.



#### EXPORT RAW DATA WITH META INFOTMATION

Raw UMI count data across all cells and their corresponding clusters and addtional meta information for clusters identified for neuronal subset data is exported for further use such as differential gene expression analysis across neuronal sub-clusters.

```{r}
## load seurat object with UMAP clustering information for neuronal sub-clusters
load("STR_SPN_SEURAT_DATA_NORM_PCA_CLUST.RData")

## extract meta information including X-Y coordinates for tSNE and UMAP
metaTemp <- as.data.frame(aaxNormClustUMAP@meta.data)
metaTemp$orig.ident <- NULL
...
metaTemp$NewCluster <- sprintf("%02d", as.numeric(metaTemp$Cluster))

metaTemp$cells <- row.names(metaTemp)

tsneProj <- as.data.frame(aaxNormClustUMAP@dr$tsne@cell.embeddings)
colnames(tsneProj) <- c("TSNE_1", "TSNE_2")
tsneProj$cells <- row.names(tsneProj)

umapProj <- as.data.frame(aaxNormClustUMAP@dr$umap@cell.embeddings)
colnames(umapProj) <- c("UMAP_1", "UMAP_2")
umapProj$cells <- row.names(umapProj)

metaDataTemp <- list(META = metaTemp, TSNE = tsneProj, UMAP = umapProj)

metaData <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "cells") } , metaDataTemp)
row.names(metaData) <- metaData$cells

## extract raw UMI count data
data.temp <- as.matrix(aaxNormClustUMAP@raw.data)
expData <- as.data.frame(t(data.temp))

## merge UMI data and meta information
aaxData <- merge(metaData, expData, by = "row.names")
row.names(aaxData) <- aaxData$Row.names
colnames(aaxData)[colnames(aaxData) == "Row.names"] <- "CellBarcode"

aaxData$cells <- NULL
aaxData$NewGeno <- NULL
aaxData$Geno <- NULL
aaxData$NewCluster <- NULL

## save RData file
save(aaxData, file = "STR_SPN_SEURAT_DATA_NORM_PCA_CLUST_RAWDATA.RData")
```
